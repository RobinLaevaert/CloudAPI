  ...
  ngOnInit() {
    ...
    this.svc.getTeamFromName(this.router.url.slice(6)).subscribe(res => {
      ...
      this.svc.getCompetition(this.competitionid).subscribe(res => {
        var games = res.numberOfMatchdays;
        this.svc.getTeam(this.id).subscribe(result => {
          ...
          this.svc.getCompetitionTable(this.competitionid, games).subscribe(result => {
            ...
            this.svc.getTeamFixtures(this.id).subscribe(result => {
              ...
              var teamsincomp;
              this.svc.getCompetitionTeams(this.competitionid).subscribe(o => {
                teamsincomp = o.teams;
                ...
                this.Fixtures.forEach(s => {
                  ...
                  if (+(s._links.competition.href.slice(45)) == +this.competitionid) {
                    teamsincomp.forEach(t => {
                      if (s.homeTeamName == "FC Bayern München" || s.awayTeamName == "FC Bayern München") {
                        ...
                        else if (s.homeTeamName == t.name) { s.crestHomeTeam = t.crestUrl }
                        ...
                        else if (s.awayTeamName == t.name) { s.crestAwayTeam = t.crestUrl }
                      }
                      else {
                        if (t.name == s.homeTeamName) { s.crestHomeTeam = t.crestUrl; }
                        if (t.name == s.awayTeamName) { s.crestAwayTeam = t.crestUrl; }
                      }
                    })
                  }
                  else {
